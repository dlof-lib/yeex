...
- **علامة مائية على كل فريمات الفيديو نفسه** (وليس الغلاف فقط): `WatermarkUtil` يعمل على
  الصور والغلاف؛ دمج علامة على كل فريم من الفيديو يحتاج مكتبة إعادة ترميز مثل `ffmpeg-kit`
  (ثقيلة الحجم، ~30-50MB إضافية على حجم التطبيق، غير مضافة افتراضياً). تصدير PDF لفقرة
  فيديو حالياً يستخدم بطاقة نصية (النص المرافق) بدل فريم فعلي من الفيديو لنفس السبب.
