...
- **حالة "أعجبني" لكل مستخدم** على الفيد (`hasLiked` في `ParagraphCard`): يحتاج قراءة من
  `/paragraphLikes/{id}/{uid}` — البنية موجودة في القواعد كتعليق، التنفيذ الكامل لم يُضَف.
- **صفحة تعليقات وحوار إعادة النشر**: الأزرار موجودة، الشاشات نفسها (bottom sheet / dialog)
  تحتاج بناء — حالياً `onOpenComments`/`onRepost` فارغة في `NavGraph.kt`.
